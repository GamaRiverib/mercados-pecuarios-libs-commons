...
export function isAdminOrWebMaster(req: any, res: any, next: any): void {
  const roles: string[] = [USER_ROLES.SUPER_ADMIN, USER_ROLES.ADMIN, USER_ROLES.WEB_MASTER];
  return validate(roles, req, res, next);
}
...
export function validatePlatformKey(req: any, res: any, next: any): void {
  const platformKey = req.query.platform;

  if (!platformKey) {
    const error = new MissingPlatformKeyError();
    writeLogWarn(req, error, { query: req.query });
    return res.status(400).json({ error });
  }

  const tenantId = PLATFORM_TENANT_ID_MAP[platformKey];

  if (!tenantId) {
    const error = new PlatformNotFoundError();
    error.platformKey = platformKey;
    writeLogWarn(req, error, { query: req.query });
